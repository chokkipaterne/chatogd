    ...
    for column_name in return_dict_cols:
        ...
        for detected_value_type in return_dict_cols[column_name]:
            ...
            if dict_tmp['score'] > threshold:
                if detected_value_type == 'booleen' and dict_tmp['score'] > 0.9:
                    return_dict_cols_intermediary[column_name].append(dict_tmp)
                elif detected_value_type != 'booleen':
                    return_dict_cols_intermediary[column_name].append(dict_tmp)
        if len(return_dict_cols_intermediary[column_name]) == 0:
            return_dict_cols_intermediary[column_name] = return_dict_cols_intermediary_full"""
        ...
        for k,v in sorted_score_types.items():
            ...
            if v == max_score:
                type_max.append(k)
            #if v < threshold and len(toremove) != len(return_dict_cols_intermediary_full):
            #    toremove.append(k)
        ...
        if ('float' in type_max or 'int' in type_max):
            if "int" in type_max and "float" in type_max and len(type_max) > 2:
                formats_to_remove.add('float')
                formats_to_remove.add('int')
            elif ("int" in type_max or "float" in type_max) and not ("int" in type_max and "float" in type_max) and len(type_max) > 1:
                formats_to_remove.add('float')
                formats_to_remove.add('int')
