...
def _is(val):
    ...
    #add by chokki (https://uibakery.io/regex-library/email-regex-python)
    regex2 = r"^[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$"
    ...
    return isValid(val) or bool(re.match(regex, val)) or bool(re.match(regex2, val)) or bool(re.match(pattern, val)) or (re.search(reg, val.lower()) is not None)
