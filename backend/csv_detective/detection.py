...
def parse_table(the_file, encoding, sep, num_rows, random_state=1):
    ...
    for encoding in [encoding, 'ISO-8859-1', 'utf-8']:
        ...
        try:
            ...
            list_null = df[df.isnull().all(axis = 1) == True].index.tolist()
            if (len(list_null) != 0):
                start = 0
                end = 0
                max_rows = 0
                ...
                max_end = 0
                i = 0
                list_null.append(df.shape[0])
                for ind in list_null:
                    end = ind
                    i += 1
                    diff = end - start
                ...
                if max_start != 0:
                    ...
                    max_rows = max_rows-1
                table = pd.read_csv(the_file, sep=sep, dtype='unicode', encoding=encoding, skiprows=max_start, nrows=max_rows)
            else:
                table = df
...
def parse_table_full(the_file, encoding, sep, num_rows, random_state=1):
    # Takes care of some problems
    table = None
    ...
    for encoding in [encoding, 'ISO-8859-1', 'utf-8']:
        ...
        try:
            ...
            list_null = df[df.isnull().all(axis = 1) == True].index.tolist()
            if (len(list_null) != 0):
                first_row_with_all_NaN = list_null[0]
                table_full = df.loc[0: first_row_with_all_NaN - 1]
            else:
                table_full = df
            #give name to empty column header
            inc = 0
            incnull = 0
            for val in table_full.columns:
                if val is None or val == "" or "Unnamed" in val:
                    table_full.columns.values[inc] = 'nocolvalue_'+str(incnull)
                    incnull += 1
                inc += 1
            total_lines = len(table_full)
            num_rows = min(num_rows - 1, total_lines)
            table = table_full.sample(num_rows, random_state=random_state)
            break
        except TypeError:
            print('Trying encoding : {encoding}'.format(encoding=encoding))
    ...
    return table, total_lines, table_full, incnull
