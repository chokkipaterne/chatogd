...
from csv_detective.utils import test_col, test_label, prepare_output_dict, is_downloadable, sort_keys, download_zipfile, download_csv
...
def return_advanced_all_tests(user_input_tests, detect_type='detect_fields'):
    """
    returns all tests that have a method _is and are listed in the user_input_tests
    the function can select a sub_package from csv_detective
    """
    ...
    if isinstance(user_input_tests, str):
        ...
        tests_to_not_do = [x[1:] for x in user_input_tests if x[0] == '-']

    all_fields = [x for x in all_packages if any([(y == x[-len(y):] and detect_type in x) for y in tests_to_do]) and all([(y != x[-len(y):] and detect_type in x) for y in tests_to_not_do])]
    all_tests = [eval(field) for field in all_fields]
    all_tests = [test for test in all_tests if '_is' in dir(test)] # TODO : Fix this shit
    return all_tests
...
def dataquality(
    ...
    with open(csv_file_path, 'r', encoding=encoding) as str_file:
        ...
        heading_columns = detect_heading_columns(str_file, sep)
        trailing_columns = detect_trailing_columns(str_file, sep, heading_columns)
        table, total_lines, table_full, nb_columns_null = parse_table_full(str_file, encoding, sep, num_rows)
    ...
    return_dict_cols = prepare_output_dict(return_table, output_mode, 0.7)
    return_dict['columns'] = return_dict_cols
    ...
    data_quality["nb_rows"] = nb_rows
    data_quality["nb_columns"] = nb_columns
    data_quality["nb_cells"] = nb_cells
    data_quality["accuracy"] = {}

    nb_incomplete_cells = np.count_nonzero(table_full.isnull())
    data_quality["nb_incomplete_cells"] = nb_incomplete_cells
    if nb_cells > 0:
        data_quality["percentage_complete_cells"] = (1 - float(nb_incomplete_cells)/float(nb_cells))*100.0

    #count number of empty rows
    nb_incomplete_rows = np.count_nonzero(table_full.isnull().any(axis=1))
    data_quality["nb_incomplete_rows"] = nb_incomplete_rows
    if nb_rows > 0:
        data_quality["percentage_complete_rows"] = (1 - float(nb_incomplete_rows)/float(nb_rows))*100.0
    
    #percentage of comprehensible columns
    if nb_columns > 0:
        data_quality["percentage_comprehensible_columns"] = (1- float(nb_columns_null)/float(nb_columns))*100.0

    nbcol_accuracy = 0
    cols_types = ""
    total_accuracy = 0
    percentage_accurate_cells = 1
    for col, type_col in return_dict['columns'].items():
        #print(col)
        if output_mode == 'ALL':
            if len(type_col) > 0:
                type_col = type_col[0]
        ...
        if type_col is None:
            cols_types +=col+"###**"
            data_quality["accuracy"][col] = 0
        if type_col is not None and type_col["format"] == "string":
            cols_types += col+"#"+str(type_col["format"])+"#"+str(type_col["score"])+"#**"
            data_quality["accuracy"][col] = 100

        if type_col is not None and type_col["format"] != "string":
            nbcol_accuracy += 1
            #print(type_col["format"])
            col_tests_fields = return_advanced_all_tests(type_col["format"], detect_type='detect_fields')
            #print(col_tests_fields)
            #col_table = table_full.loc[:,col]
            col_table = table_full[[col]]
            col_return_table_fields = test_col(col_table, col_tests_fields, total_lines, output_mode)
            #print(col_return_table_fields)
            if len(col_return_table_fields) != 0:
                accuracy_col = col_return_table_fields[col].iloc[0]
            total_accuracy += accuracy_col
            cols_types += col+"#"+str(type_col["format"])+"#"+str(type_col["score"])+"#"+str(accuracy_col)+"**"
            data_quality["accuracy"][col] = 100 * float(accuracy_col)

    if nbcol_accuracy > 0:
        percentage_accurate_cells = float(total_accuracy)/float(nbcol_accuracy)

    data_quality["percentage_accurate_cells"] = percentage_accurate_cells*100.0
    data_quality["nbcol_accuracy"] = nbcol_accuracy
    data_quality["cols_types"] = cols_types
    data_quality["return_dict"] = return_dict

    if save_results:
        # Write your file as json
        output_path_to_store_minio_file = os.path.splitext(csv_file_path)[0] + '_detect.json'
        with open(output_path_to_store_minio_file, 'w', encoding='utf8') as fp:
            json.dump(data_quality, fp, indent=4, separators=(',', ': '))

    return data_quality

#Get data quality of dataset
def data_quality_dataset(save_path, num_rows):
    #url = "https://gnb.socrata.com/api/views/k2yj-tprx/rows.csv?accessType=DOWNLOAD"
    data_quality = {'error': True}
    inspection_results = sort_keys(dataquality(save_path, num_rows=num_rows, output_mode='ALL', save_results=True))
    if inspection_results and "percentage_accurate_cells" in inspection_results and inspection_results["nb_cells"] > 0:
        data_quality = inspection_results
    return data_quality
